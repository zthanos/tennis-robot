...
### Η γέφυρα λείπει εντελώς — και είναι το επίπεδο διαχωρισμού

Το CAD έχει **18 mm plywood portal bridge** (`option-a.scad`) που χωρίζει τα δύο
συγκροτήματα, και ο launcher πατάει **πάνω** σε αυτήν:

```text
oa_bridge_center_x 490, depth 220, width 470   -> x[380,600] y[+/-235]
oa_bridge_under_z  150,  oa_bridge_t 18        -> z[150,168]
oa_cheek_top_z = oa_bridge_under_z             -> τα cheeks ΣΤΑΜΑΤΟΥΝ στα 150
```

`grep -rn "bridge" ros2_ws/src/tennis_robot/urdf/` δεν βρίσκει τίποτα: **η
γέφυρα δεν μοντελοποιείται καθόλου** στο URDF.

Και το `funnel_link` του URDF φτάνει σε `z = 281 mm` — **131 mm ψηλότερα** από
το `oa_cheek_top_z = 150` του CAD. Δηλαδή τα cheeks περνούν ίσια μέσα από το
επίπεδο της γέφυρας, μέσα στον όγκο του launcher. Αυτή είναι η πραγματική πηγή
των υπολειπόμενων διεισδύσεων: με διορθωμένο cradle αλλά ανέπαφο funnel
μετρήθηκαν `frame vs funnel 58.5 mm`, `wheels vs funnel 36.5 mm`.

### Τρεις ανεξάρτητες αποκλίσεις CAD ↔ URDF

1. **frame**: πλάκα 280x508x35 στα z[24,152] αντί για δύο ελάσματα 8 mm
   στα z[127,303]·
2. **bridge**: δεν υπάρχει καθόλου στο URDF (CAD: z[150,168], φέρει τον launcher)·
3. **funnel cheeks**: `z_max 281` στο URDF αντί για `150` στο CAD.

Δοκιμάστηκε μόνο η (1) — αντικατάσταση της πλάκας με το CAD cradle. Δεν αρκεί:
τα cheeks εξακολουθούν να διεισδύουν (58.5 mm) και εμφανίζεται **νέα** σύγκρουση
`basket_link <-> flywheel_launcher_frame_link 6.6 mm` στο πλήρες lift, επειδή
και το basket του URDF διαφέρει από το CAD. Η αλλαγή **αναιρέθηκε**: μισο-
μεταφερμένο μοντέλο με νέα διείσδυση είναι χειρότερο από τεκμηριωμένο ελάττωμα.

**ΑΝΟΙΧΤΟ — απαιτεί απόφαση εύρους.** Η αποκατάσταση της συνύπαρξης δεν είναι
μία αλλαγή· είναι ανακατασκευή του μπροστινού συγκροτήματος από το CAD:
γέφυρα + ύψος cheeks + έδραση launcher πάνω στη γέφυρα + cradle. Μέχρι τότε το
`option-a-launch` αφαιρεί το intake για λόγο που **δεν προκύπτει από το σχέδιο**,
και η δήλωση «intake και launcher δεν συνυπάρχουν» πρέπει να θεωρείται
**ιδιότητα του URDF, όχι της μηχανής**.
