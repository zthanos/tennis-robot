...
- **LiDAR-first.** Ό,τι είναι κάθετο (φράχτες, δίχτυ, στύλοι, εμπόδια) μετριέται από το
  360° LiDAR. Κάμερα ΔΕΝ χρησιμοποιείται στη Φάση 1.
...
- **FIND_NET:** οδήγηση μπροστά μέχρι το δίχτυ· **net lock** από το μπροστινό LiDAR
  range (+ vision net class) → ορίζει το court frame (origin = net center, +x' =
  robot→net, +y' = κατά μήκος του διχτιού).
