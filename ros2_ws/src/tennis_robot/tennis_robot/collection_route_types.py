...
class ExecutionProfile:
    nominal_speed_mps: float; min_speed_mps: float; max_speed_mps: float
    ...
    def __post_init__(self) -> None:
        for name in ("nominal_speed_mps", "min_speed_mps", "max_speed_mps", "max_acceleration_mps2", "max_deceleration_mps2", "required_entry_m", "required_run_in_m", "required_run_out_m", "max_curvature_per_m", "max_lateral_error_m", "max_heading_error_rad"):
...
class CollectionRouteConfiguration:
    schema_version: str; mechanical: MechanicalConfiguration; uncertainty: UncertaintyConfiguration; safety: SafetyConfiguration; scan: ScanConfiguration; planning: PlanningConfiguration
    def __post_init__(self) -> None:
        _non_empty(self.schema_version, "schema_version")
        for name, typ in (("mechanical", MechanicalConfiguration), ("uncertainty", UncertaintyConfiguration), ("safety", SafetyConfiguration), ("scan", ScanConfiguration), ("planning", PlanningConfiguration)):
            if not isinstance(getattr(self, name), typ): raise DomainValidationError(f"{name} must be {typ.__name__}")
    def to_dict(self) -> dict[str, Any]: return {"schema_version": self.schema_version, "mechanical": self.mechanical.to_dict(), "uncertainty": self.uncertainty.to_dict(), "safety": self.safety.to_dict(), "scan": self.scan.to_dict(), "planning": self.planning.to_dict()}
    @classmethod
    def from_dict(cls: type[_T], data: Mapping[str, Any]) -> _T:
        _fields(data, set(cls.__dataclass_fields__), "CollectionRouteConfiguration")
        return cls(data["schema_version"], MechanicalConfiguration.from_dict(data["mechanical"]), UncertaintyConfiguration.from_dict(data["uncertainty"]), SafetyConfiguration.from_dict(data["safety"]), ScanConfiguration.from_dict(data["scan"]), PlanningConfiguration.from_dict(data["planning"]))
...
@dataclass(frozen=True)
class RouteSegment:
    id: str; type: RouteSegmentType; path: Path2D; progress_start_m: float; progress_end_m: float; execution_profile: ExecutionProfile; covered_ball_ids: tuple[str, ...]; obstacle_constraint: ObstacleConstraint
    def __post_init__(self) -> None:
        ...
        elif self.covered_ball_ids: raise DomainValidationError("only funnel passes may cover balls")
    ...
    def to_dict(self) -> dict[str, Any]: return {"id": self.id, "type": self.type.value, "path": self.path.to_dict(), "progress_start_m": self.progress_start_m, "progress_end_m": self.progress_end_m, "execution_profile": self.execution_profile.to_dict(), "covered_ball_ids": list(self.covered_ball_ids), "obstacle_constraint": self.obstacle_constraint.to_dict()}
    @classmethod
    def from_dict(cls: type[_T], data: Mapping[str, Any]) -> _T:
        _fields(data, set(cls.__dataclass_fields__), "RouteSegment")
        if not isinstance(data["covered_ball_ids"], list): raise DomainValidationError("covered_ball_ids must be a JSON array")
        return cls(data["id"], _enum(data["type"], RouteSegmentType, "type"), Path2D.from_dict(data["path"]), data["progress_start_m"], data["progress_end_m"], ExecutionProfile.from_dict(data["execution_profile"]), tuple(data["covered_ball_ids"]), ObstacleConstraint.from_dict(data["obstacle_constraint"]))


@dataclass(frozen=True)
class CollectionRoutePlan:
    plan_id: str; scan_id: str; map_frame: str; start_pose: Pose2D; terminal_pose: Pose2D; total_length_m: float; expected_duration_s: float; planning_status: PlanningStatus; segments: tuple[RouteSegment, ...]; snapshot_ball_ids: tuple[str, ...]; ball_results: tuple[BallResult, ...]; configuration_snapshot: CollectionRouteConfiguration
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.start_pose, Pose2D) or not isinstance(self.terminal_pose, Pose2D) or not isinstance(self.planning_status, PlanningStatus) or not isinstance(self.configuration_snapshot, CollectionRouteConfiguration): raise DomainValidationError("invalid plan fields")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"plan_id": self.plan_id, "scan_id": self.scan_id, "map_frame": self.map_frame, "start_pose": self.start_pose.to_dict(), "terminal_pose": self.terminal_pose.to_dict(), "total_length_m": self.total_length_m, "expected_duration_s": self.expected_duration_s, "planning_status": self.planning_status.value, "segments": [segment.to_dict() for segment in self.segments], "snapshot_ball_ids": list(self.snapshot_ball_ids), "ball_results": [result.to_dict() for result in self.ball_results], "configuration_snapshot": self.configuration_snapshot.to_dict()}
    @classmethod
    def from_dict(cls: type[_T], data: Mapping[str, Any]) -> _T:
        ...
        return cls(data["plan_id"], data["scan_id"], data["map_frame"], Pose2D.from_dict(data["start_pose"]), Pose2D.from_dict(data["terminal_pose"]), data["total_length_m"], data["expected_duration_s"], _enum(data["planning_status"], PlanningStatus, "planning_status"), tuple(RouteSegment.from_dict(segment) for segment in data["segments"]), tuple(data["snapshot_ball_ids"]), tuple(BallResult.from_dict(result) for result in data["ball_results"]), CollectionRouteConfiguration.from_dict(data["configuration_snapshot"]))
