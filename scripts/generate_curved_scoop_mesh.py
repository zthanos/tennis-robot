...
# Geometry (m, ground frame). The launch profile matches the opt-in Gazebo
# collision generated by generate_robot_urdf.py.
...
_ENTRY_DEFAULT_M = _NIP_X_M if RAMP_PROFILE == "launch" else _NIP_X_M - 0.040
LIP_X = float(os.getenv("INTAKE_RAMP_ENTRY_X_M", str(_ENTRY_DEFAULT_M)))
...
ROLLING_RAMP_END_X = float(os.getenv("INTAKE_RAMP_END_X_M", "0.425"))
LAUNCH_EXIT_X = float(os.getenv("INTAKE_LAUNCH_EXIT_X_M", "0.465"))
LAUNCH_EXIT_Z = float(os.getenv("INTAKE_LAUNCH_EXIT_Z_M", "0.032"))
LAUNCH_EXIT_ANGLE_DEG = float(os.getenv("INTAKE_LAUNCH_EXIT_ANGLE_DEG", "35.0"))
RAMP_END_X = LAUNCH_EXIT_X if RAMP_PROFILE == "launch" else ROLLING_RAMP_END_X
...
def ramp_z(x: float) -> float:
    ...
    if RAMP_PROFILE == "launch":
        ...
        h01 = -2.0 * t**3 + 3.0 * t**2
        h11 = t**3 - t**2
        return h00 * LIP_HEIGHT_M + h01 * LAUNCH_EXIT_Z + h11 * m1
    if x >= RAMP_CLEAR_X:
