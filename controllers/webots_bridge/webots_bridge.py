...
KINEMATIC_BASE = os.getenv("WEBOTS_BRIDGE_KINEMATIC_BASE", "true").lower() not in {"0", "false", "no"}
...
class WebotsBridgeNode(Node):
    def __init__(self, robot: Supervisor) -> None:
        ...
        self._translation_field = self._robot_node.getField("translation")
        self._rotation_field = self._robot_node.getField("rotation")
        self._yaw_rad = self._read_yaw_rad()
        ...
        self._last_linear_m_s = 0.0
        self._last_angular_rad_s = 0.0
        self._last_left_motor_rad_s = 0.0
        self._last_right_motor_rad_s = 0.0
        self._last_cmd_log_s = 0.0
    ...
    def _on_cmd_vel(self, msg: Twist) -> None:
        self._last_linear_m_s = float(msg.linear.x)
        self._last_angular_rad_s = float(msg.angular.z)
        self._apply_base_command()

    def _apply_base_command(self) -> None:
        linear = self._last_linear_m_s
        angular = self._last_angular_rad_s
        ...
        self._last_left_motor_rad_s = max(-MAX_SPEED_RAD_S, min(MAX_SPEED_RAD_S, right_wheel))
        self._last_right_motor_rad_s = max(-MAX_SPEED_RAD_S, min(MAX_SPEED_RAD_S, left_wheel))
        self._left_motor.setVelocity(self._last_left_motor_rad_s)
        self._right_motor.setVelocity(self._last_right_motor_rad_s)

        now = time.time()
        if now - self._last_cmd_log_s >= 2.0 and (abs(linear) > 1e-6 or abs(angular) > 1e-6):
            pos = self._robot_node.getPosition()
            self.get_logger().info(
                "cmd_vel linear=%.3f angular=%.3f -> motors left=%.3f right=%.3f pose=(%.3f, %.3f)"
                % (
                    linear,
                    angular,
                    self._last_left_motor_rad_s,
                    self._last_right_motor_rad_s,
                    pos[0],
                    pos[1],
                )
            )
            self._last_cmd_log_s = now
    ...
    def step(self, dt_s: float) -> None:
        ...
        self._apply_base_command()
        self._update_kinematic_base(dt_s)
    ...
    def _read_yaw_rad(self) -> float:
        rot = self._rotation_field.getSFRotation()
        if abs(rot[2]) < 0.5:
            return 0.0
        return float(rot[3]) if rot[2] >= 0 else -float(rot[3])

    def _update_kinematic_base(self, dt_s: float) -> None:
        if not KINEMATIC_BASE:
            return
        linear = self._last_linear_m_s
        angular = self._last_angular_rad_s
        ...
        self._yaw_rad = (self._yaw_rad + angular * dt_s + math.pi) % (2 * math.pi) - math.pi
        pos = self._translation_field.getSFVec3f()
        pos[0] += linear * math.cos(self._yaw_rad) * dt_s
        pos[1] += linear * math.sin(self._yaw_rad) * dt_s
        self._translation_field.setSFVec3f(pos)
        self._rotation_field.setSFRotation([0.0, 0.0, 1.0, self._yaw_rad])
